...
public class RoleController extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        switch (op) {
            case "load":
                response.getWriter().write(g.toJson(rs.findAll()));
                break;
            case "add":
                code = request.getParameter("code");
                libelle = request.getParameter("libelle");
                rs.create(new Role(code, libelle));
                response.getWriter().write(g.toJson(rs.findAll()));
                break;
            case "delete":
                id = request.getParameter("id");
                rs.delete(rs.findById(Integer.parseInt(id)));
                response.getWriter().write(g.toJson(rs.findAll()));
                break;
            case "update":
                id = request.getParameter("id");
                code = request.getParameter("code");
                libelle = request.getParameter("libelle");
                Role r = rs.findById(Integer.parseInt(id));
                r.setCode(code);
